CREATE FUNCTION pg_temp.coalesce_agg_sfunc(state int, value int) RETURNS int AS
$$
    SELECT coalesce(value, state);
$$ LANGUAGE SQL;

CREATE AGGREGATE pg_temp.coalesce_agg(int) (
    SFUNC = pg_temp.coalesce_agg_sfunc,
    STYPE  = int);


with inputs as(
  select '[1518-03-11 00:45] wakes up
...
[1518-08-01 00:40] falls asleep'::text as inputs
)
,split as (
...
,id_split as (
  ...
  select row_number() over() as row_id, inputs as raw_inputs,
    ((regexp_matches(inputs,'\[(.*)\]'))[1])::timestamp as event_time,
    ((regexp_matches(inputs,'Guard #(\d+) '))[1])::int as guard_id,
    case when inputs ~* 'wakes up$' then 1 else 0 end as wake_up_event,
    case when inputs ~* 'asleep$' then 1 else 0 end as sleep_down_event
--     ((regexp_matches(inputs,': (\d+)x'))[1])::int as top_size,
--     ((regexp_matches(inputs,'x(\d+)'))[1])::int as left_size,
--     ((regexp_matches(inputs,': (\d+)x'))[1])::int *
--     ((regexp_matches(inputs,'x(\d+)'))[1])::int as size
  from split
)
,guards as(
select row_id,
...
--   guard_id,
  ...
  wake_up_event,
  sleep_down_event,
  pg_temp.coalesce_agg(guard_id) over (order by event_time) as guard_id
  from id_split
-- group by 1,2,3,4,5
-- order by event_time
)
,lags as (
    select
      *,
      lag(event_time)
      over (
        partition by guard_id
        order by event_time ),
      extract('minute' from lag(event_time)
      over (
        partition by guard_id
        order by event_time )) as sleep_start,
      extract('epoch' from event_time - lag(event_time)
      over (
        partition by guard_id
        order by event_time ))/60 as time_dif
    from guards
)
,lag_minutes as (  select
                     *,
                     generate_series(sleep_start :: int, sleep_start :: int + time_dif :: int - 1) as minutes
                   from lags
)
,worst_guard as (select
                   guard_id,
                   count(1)
                 from lag_minutes
                 where wake_up_event = 1
                 group by 1
                 order by 2 desc
limit 1
)
,part1 as (select
             ...
             count(1)
           from lag_minutes
             inner join worst_guard using (guard_id)
           where wake_up_event = 1
           group by 1, 2
           order by 3 desc
           limit 1
